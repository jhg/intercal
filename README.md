...
| `tests/test_bytecode.sh` | 24 | Bytecode tier v4 (arithmetic, COME FROM, NEXT/RESUME, ABSTAIN, multi-dim arrays, TTM, WRITE IN, syslib eval, Label 666 file I/O) |
| `tests/test_bytecode_equiv.sh` | 16 of 35 | Native vs bytecode equivalence sweep |
| `tests/test_emit_help.sh` | 23 | `--emit-help` self-documenting CLI |
...
| `tests/test_stage3_substage1.sh` | 4 | stage3 byte loader + 'D' counter + 'P' counter |
...
- Want to contribute to a production compiler after reading this? See [docs/from-intercal-to-real-compilers.md](docs/from-intercal-to-real-compilers.md) and the eleven chapters of Part VII (LLVM, GCC, rustc, Go, GHC, OCaml, Cranelift, Zig, Swift, V8, plus the contributing chapter).
