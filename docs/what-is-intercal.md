...
- **Because the language is a useful proving ground for AI-driven development.** Most of this repository was written by a large language model paired with one human collaborator: the human set the goals, made the first commits, supplied references, and reviewed; the compiler, runtime, syslib, tests, and most of this documentation came from the model. INTERCAL fits the experiment well. It is small enough that the model can hold the whole thing in context, weird enough that off-the-shelf code patterns rarely apply, and specified well enough that any divergence is detectable. A more conventional language would let the model lean on recall; a much smaller spec would not exercise enough of the work involved in building a compiler.
