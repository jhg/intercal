...
_stmt_2:  // STASH
  ...
  adrp x0, _spot_1_stash_ptr@PAGE
  add x0, x0, _spot_1_stash_ptr@PAGEOFF
  ldr x1, [x0]
  cbnz x1, .Lstash_ok_1
  stp x0, x30, [sp, #-16]!
  mov x0, #4096
  ...
  mov x1, x0
.Lstash_ok_1:
  adrp x2, _spot_1_stash_sp@PAGE
  add x2, x2, _spot_1_stash_sp@PAGEOFF
  ldr w3, [x2]
  cmp w3, #1023
  b.gt _rt_error_E000
  adrp x4, _spot_1@PAGE
  add x4, x4, _spot_1@PAGEOFF
  ldr w5, [x4]
  str w5, [x1, x3, lsl #2]
  add w3, w3, #1
  str w3, [x2]
